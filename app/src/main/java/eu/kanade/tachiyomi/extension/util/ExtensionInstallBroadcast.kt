...
class ExtensionInstallBroadcast : BroadcastReceiver() {
    override fun onReceive(
        ...
        try {
            ...
            val data = intent.data?.let { UniFile.fromUri(context, it).openInputStream() } ?: return
                ...
                when (val status = extras.getInt(PackageInstaller.EXTRA_STATUS)) {
                    ...
                    PackageInstaller.STATUS_FAILURE, PackageInstaller.STATUS_FAILURE_ABORTED, PackageInstaller.STATUS_FAILURE_BLOCKED, PackageInstaller.STATUS_FAILURE_CONFLICT, PackageInstaller.STATUS_FAILURE_INCOMPATIBLE, PackageInstaller.STATUS_FAILURE_INVALID, PackageInstaller.STATUS_FAILURE_STORAGE -> {
                        ...
                        if (status != PackageInstaller.STATUS_FAILURE_ABORTED) {
                            ...
                            } else if (DeviceUtil.isHyperOS) {
                                // HyperOS blocks PackageInstaller sessions for non-system apps.
                                // Prompt user to use Shizuku or disable HyperOS app verification.
                                context.toast(R.string.extensions_hyperos_warning, Toast.LENGTH_LONG)
                            } else {
...
class ExtensionInstallActivity : Activity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            val data = intent.data?.let { UniFile.fromUri(this, it).openInputStream() } ?: return
