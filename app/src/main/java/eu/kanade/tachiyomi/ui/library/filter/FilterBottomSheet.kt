...
class FilterBottomSheet
    ...
    constructor(
        ...
        private fun hasActiveFiltersFromPref(): Boolean =
            preferences.filterDownloaded().get() > 0 ||
        ...
        private fun addForClear(): Int = if (clearButton.parent != null) 1 else 0

        private fun mapOfFilters(char: Char): FilterTagGroup? =
            when (Filters.filterOf(char)) {
        ...
        private fun setFilterPreference(
            view: FilterTagGroup,
            index: Int,
        ) {
            when (view) {
