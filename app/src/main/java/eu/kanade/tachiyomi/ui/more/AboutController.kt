...
class AboutController : SettingsController() {
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) =
        screen.apply {
            ...
            preference {
                key = "pref_build_version"
                title = "Build"
                summary = BuildConfig.BUILD_VERSION
                onClick {
                    activity?.let {
                        val clipboard = it.getSystemService<ClipboardManager>()!!
                        clipboard.setPrimaryClip(ClipData.newPlainText("Build Version", BuildConfig.BUILD_VERSION))
                        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU) {
                            view?.snack(context.getString(R.string._copied_to_clipboard, "Build Version"))
                        }
                    }
                }
            }
